...
The project uses Provider for state management and follows a modular widget structure for better readability and maintainability. Large JSON data is processed using Dart's `Isolate.spawn()` to keep the user interface responsive.

The application also handles loading, empty, network error, API status code errors, and general error states to provide a smooth and reliable user experience.
...
- Background JSON parsing using `Isolate.spawn()`

- Loading and empty states

- Network and general error handling

- HTTP status code handling for 401, 404, and 500 responses
...
- **Dart Isolates** – Background JSON parsing using `Isolate.spawn()`
...
The API response is converted into `NotificationModel` objects before being provided to the user interface. Since the response contains a large amount of data, JSON parsing is performed in a separate isolate using Dart's `Isolate.spawn()` to help keep the UI responsive.

A `ReceivePort` and `SendPort` are used for communication between the main isolate and the worker isolate. After parsing the JSON data, the worker isolate sends the parsed notification data back to the main isolate.
...
- HTTP status codes are handled for authentication errors (401), missing resources (404), and server errors (500).
...
- **`Isolate.spawn()`** is used to parse the large notification JSON response in a separate isolate without blocking the main UI isolate.

- **`ReceivePort` and `SendPort`** are used for communication between the main isolate and the worker isolate.
